...
class NewsController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate() ) {
            ...
            // Save Image
            $model->image=$this->generalObj->saveFile('news',$model,$model->title);
            ...
            // Order Recorder
            $max = News::find()->select('max(id)')->scalar()+1;
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post()) ) {
            ...
            // Update Image
            $model->image=$this->generalObj->updateFile('news',$model,$oldImage,$model->title);
    ...
    public function actionUpdateOrder(){
        ...
        foreach($modelValueQuery[0] as $i=>$result ){
                ...
                if ($result->load(Yii::$app->request->post()) )
                {
                    ...
                    $modelDataOld = News::find()->where(['order_recorder' => $arrayPost['News']['old_order_recorder']])->one();

                    $modelDataNew = News::find()->where(['order_recorder' => $arrayPost['News']['order_recorder']])->one();

                    $model1 = $this->findModel($modelDataOld['id']);
                    $model1->order_recorder = $arrayPost['News']['order_recorder'];
                    $model1->save();

                    $model2 = $this->findModel($modelDataNew['id']);
                    $model2->order_recorder = $arrayPost['News']['old_order_recorder'];
                    $model2->save();
