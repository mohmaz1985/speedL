...
<div class="form">
    ...
    <?
        ...
        foreach($model as $i => $result ){ ?>
          ...
          <td >
            <?php
              $dataList=ArrayHelper::map(Contact::find()->where(['lang' => Yii::$app->language])->asArray()->all(), 'order_recorder', 'order_recorder');
              echo $form->field($result, "order_recorder")->dropDownList($dataList, 
                       ['prompt'=>'-swap with-']) ->label(false)
            ?>
          </td>
          <td>
            <?= Html::a(Yii::t('yii','Update'), ['contact/update-order', 'id' => $result->id], [
          ...
          <td >
          <?= Html::a(Yii::t('yii','Delete From Database'), ['contact/delete-from-database', 'id' => $result->id], [
          ...
          <td >
            <?= Html::a(Yii::t('yii','Recover'), ['contact/recover', 'id' => $result->id], [
